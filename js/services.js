(function(){
  ...
  // Already shown in the "Signature Service" spotlight above — don't list it twice.
  const FEATURED_NAME = 'haircut';
  ...
  async function load(){
    ...
    const list = (data || []).filter(s => (s.name || '').trim().toLowerCase() !== FEATURED_NAME);
    if(list.length===0){
